	 					...
	 					<c:if test="${user.perfil.equals('administrador')}">
	 						<c:choose>
	 							<c:when test="${aula.numaula!=null}">
	 								...
	 								<a class="btn btn-danger" title="Eliminar" href="aula?action=eliminar&numaula=${aula.numaula}" onclick="return confirm('Se va a proceder a la eliminaci�n del aula. �Desea continuar?')">Eliminar</a>
	 							</c:when>
	 							<c:otherwise>
